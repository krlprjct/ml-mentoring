...
// Немного смягчил пружину для более плавного "разжимания"
const IOS_SPRING = { 
  type: "spring", 
  stiffness: 250, 
  damping: 25, 
...
const Navbar = memo(() => {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    // Добавил запас (гистерезис), чтобы не дергалось на границе
    ...
    <div className="fixed top-6 left-0 w-full z-50 flex justify-center pointer-events-none px-4">
      ...
      >
        ...
        <AnimatePresence>
          {!isScrolled && (
            <motion.div
              // layout prop здесь критичен для плавного расширения родителя
              layout
              ...
              style={{ whiteSpace: 'nowrap' }} // Чтобы текст не прыгал вниз при анимации
            >
              <div className="flex gap-1 px-2">
                {['Манифест', 'Результаты', 'Путь', 'Тарифы'].map((item) => (
                  <a 
                    ...
                    className="px-3 py-2 rounded-full text-[10px] md:text-xs font-bold uppercase hover:bg-white transition-colors text-black/70"
                  >
                    {item}
                  </a>
                ))}
